...
discoInfoFeature :: Text
discoInfoFeature = "http://jabber.org/protocol/disco#info"

discoInfoName :: Text -> Name
discoInfoName = nsName discoInfoFeature
...
parseDiscoEntity re = do
  ...
  let discoFeatures = S.insert discoInfoFeature discoFeatures'
...
instance (MonadStream m) => Codec m FullJID Presence (DiscoEntityCodec m) where
  codecDecode (DiscoEntityCodec pluginsRef) faddr pres = do
    ...
    lazy <- LazyOnce.new $ forceGetDiscoEntitySync pluginsRef addr Nothing
...
-- | Always perform a fresh disco#info request (no caching).
forceGetDiscoEntity :: (MonadStream m) => XMPPPluginsRef m -> XMPPAddress -> Maybe DiscoNode -> (Either StanzaError DiscoEntity -> m ()) -> m ()
forceGetDiscoEntity pluginsRef addr node handler = do
...
-- | Synchronous wrapper around forceGetDiscoEntity, for use inside LazyOnce.
forceGetDiscoEntitySync :: (MonadStream m) => XMPPPluginsRef m -> XMPPAddress -> Maybe DiscoNode -> m (Either StanzaError DiscoEntity)
forceGetDiscoEntitySync pluginsRef addr node = do
  resultVar <- newEmptyMVar
  forceGetDiscoEntity pluginsRef addr node $ putMVar resultVar
  takeMVar resultVar
      ...
      case M.lookup full presences of
        Just pres
          | Just (LazyDiscoEntity lazy) <- Reg.lookup (Proxy :: Proxy (LazyDiscoEntity m)) (presenceExtended pres) ->
              LazyOnce.get lazy >>= handler
        _ -> forceGetDiscoEntity pluginsRef addr node handler
  | otherwise = forceGetDiscoEntity pluginsRef addr node handler
  where
...
discoItemsFeature :: Text
discoItemsFeature = "http://jabber.org/protocol/disco#items"

discoItemsName :: Text -> Name
discoItemsName = nsName discoItemsFeature
...
getDiscoTopo pluginsRef addr node handler = do
  ...
  getDiscoEntity pluginsRef addr node $ \case
    Left e -> do
      ...
      handler $ Left e
    Right discoRoot | discoItemsFeature `S.member` discoFeatures discoRoot ->
...
discoPlugin pluginsRef = do
  ...
  homeLazy <- LazyOnce.new $ forceGetDiscoEntitySync pluginsRef homeAddr Nothing
