...
sourceConn conn =
  lift (handleConnErr $ connectionGetChunk conn) >>= \case
    "" -> return ()
...
data ConnectionInterruptionException
...
unexpectedStanza got expected =
  undefinedError
    { smeType = StInvalidXml
    , smeText =
        Just $
          [i|Unexpected stanza: got <#{nameLocalName got}>, expected one of |]
...
streamRecv' stream@(Stream {..}) = modifyMVar streamSource $ \source ->
 ...
 where
  internalError (InternalStreamException e) = streamThrow stream e

streamRecv :: (MonadStream m) => Stream m -> m Element
streamRecv stream =
  streamRecv' stream >>= \case
